...
**** Changed the function src_verilog{$stop} in [[file:Source/Chapter%204/openmips_min_sopc_tb.v][openmips_min_sopc_tb.v]] to src_verilog{$finish} so that VVP could be dismissed and also added the timescale in  [[file:Source/Chapter%204/openmips_min_sopc_tb.v:5][openmips_min_sopc_tb.v]].
...
*** Question: How did src_verilog{shiftres <= ({32{reg2_i[31]}} << (6'd32-{1'b0, reg1_i[4:0]}))| reg2_i >> reg1_i[4:0];} implemented arithmetic left shift?
