...
Not only I didn't had the warning "Not enough words in the file" as the fuction src_<verilog>{$readmemh} would read the depth of "inst_mem" hexadecimal numbers in [[file:Source/Chapter%204/inst_rom.data][inst_rom.data]], but also the time for compilation was much shortened.
**** Changed the function src_<verilog>{$stop} in [[file:Source/Chapter%204/openmips_min_sopc_tb.v][openmips_min_sopc_tb.v]] to src_<verilog>{$finish} so that VVP could be dismissed and also added the timescale in  [[file:Source/Chapter%204/openmips_min_sopc_tb.v:5][openmips_min_sopc_tb.v]].
...
*** Question: How did src_<verilog>{shiftres <= ({32{reg2_i[31]}} << (6'd32-{1'b0, reg1_i[4:0]}))| reg2_i >> reg1_i[4:0];} implemented arithmetic left shift?
